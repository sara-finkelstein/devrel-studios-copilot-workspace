...
- **Den tech check still awaiting response:** Allison Dunmire emailed Den Delimarsky and Harald Kirschner about the mandatory 15-minute MCP Live! tech check on August 18, followed up with both on August 21 and 25, and had not received Den's reply in the thread found. Harald selected Friday, September 4 at 9:15 AM Pacific and received his invite.
- **On-demand delivery setup completed 9/1:** all eight Post Event Episodes ([242418](https://dev.azure.com/devrel/Studios/_workitems/edit/242418) through [242425](https://dev.azure.com/devrel/Studios/_workitems/edit/242425)) now publish Tuesday, September 15, 2026 at 9:00 AM Pacific. Thumbnail requirements are enabled, creating one Thumbnails child per Episode; final artwork should wait for completed titles, while Peggy's existing artwork can be reused where it still matches.
- **BOC reservation history completed 9/1:** [BOC Request 242464](https://dev.azure.com/devrel/Studios/_workitems/edit/242464) now contains the complete 17-email Bookit/BOC correspondence from August 24 through September 1, one email per Discussion comment in chronological order. Entries use compact names-only headers, substantive message text without routine signatures, exact Outlook IDs, source links, and deduplication.
- **Catering follow-up:** when Aurea returns the completed Eventions form, send the final form to catering and record the handoff in [General 240003 — Catering](https://dev.azure.com/devrel/Studios/_workitems/edit/240003). This must be completed before the September 9 MCP Live! event.
...
### FabCon Conference — ROS to ADO intake
- **Waiting on source and approval:** Ricardo has been asked whether the ADO Episodes should be created after the Fabric team returns the completed run of show. Do not create them until that confirmation and completed ROS are available.
- **Planned intake:** use the Fabric team's existing spreadsheet as a read-only source, extract only ADO-relevant fields, and retain FabCon session numbers such as `101`, `102`, and `103` in ADO for traceability. The team does not need ADO work item IDs written back into its workbook.
...
- **Stakeholder View retained; weekly messages canceled 9/4:** the shared [FY27 Fabric Production Tracker](https://microsoft-my.sharepoint.com/personal/sfinkelstein_microsoft_com/_layouts/15/Doc.aspx?sourcedoc=%7B86F378C0-619E-464E-97DA-9B300F436624%7D&file=Fabric%20Production%20Tracker.xlsx&action=default&mobileredirect=true&DefaultItemOpen=1) keeps its color-coded Stakeholder View with neutral status language and direct ADO links for possible future use. Alyssa and Terence did not want recurring status messages, so the Monday Outlook-draft automation was removed.
- **Yitzhak Kesselman short 237659 — delivered 9/3:** final delivery was emailed to Swetha, Terence, and Alyssa using the Ron Chang handoff pattern, and the exact source-linked Sent Items message was recorded in [Episode 237659](https://dev.azure.com/devrel/Studios/_workitems/edit/237659). The burned-in final is `https://f.io/QfNIeeGU`; proofed SRT/VTT/TTML files are attached. The Episode is in `Publication` and Sara's work is complete; status is `Waiting — Fabric publishing`.
- **Bogdan Crivat/Patrice Pelland review sent 9/3:** the V1 review for [Episode 233043](https://dev.azure.com/devrel/Studios/_workitems/edit/233043) uses [Frame.io cut 8VmpyOdG](https://f.io/8VmpyOdG). Sara sent the addressed review email to Swetha, Terence, and Alyssa with feedback requested by September 4 at 9:00 AM Pacific; the exact sent email is recorded in ADO Discussion.
- **Tracker and calendar refreshed 9/2:** updated the protected [FY27 Fabric Production Tracker](https://microsoft-my.sharepoint.com/personal/sfinkelstein_microsoft_com/_layouts/15/Doc.aspx?sourcedoc=%7B86F378C0-619E-464E-97DA-9B300F436624%7D&file=Fabric%20Production%20Tracker.xlsx&action=default&mobileredirect=true&DefaultItemOpen=1) from live ADO and reconciled all current/future skill-owned Outlook appointments. Kim/Nathalie 232500 and Kanwal/Yitzhak 233550 remain waiting for final approval.
- **Transcript caption conformance awaiting review 9/4:** [PR #247](https://github.com/microsoft/mvp-copilot-plugins/pull/247) adds a deterministic `transcript-proofread` formatter enforcing 34 characters per line and two lines per cue, protected product/presenter phrases, balanced cue splitting, minimum readable split durations, synchronized SRT/VTT/TTML output, and validation-only mode. Technical review and repository checks pass; Golnaz is the requested reviewer, and Sara sent her the review-request email on September 4. Tracked by [Issue #245](https://github.com/microsoft/mvp-copilot-plugins/issues/245).
- **Fabric Production Tracker established 9/1:** the shared tracker provides one milestone-based view across FEI and FTTF while ADO remains authoritative. The current link is saved in both FY27 parent Show descriptions.
- **Content-calendar reconciliation rule confirmed 9/4:** Fabric Shorts do not belong in the Azure Data SharePoint content calendar and must be excluded from missing-record reports. Their correct planning destination remains unresolved; do not create placeholders or infer one.
- **Weekly content-calendar audit scheduled 9/4:** a durable Monday 10:00 AM Pacific task compares dated FY27 FEI/FTTF Episodes in ADO with the Azure Data SharePoint content calendar, excludes Shorts and publication dates before August 5, 2026, and sends Sara a private exception table or all-clear report without modifying either system.
- **Content-calendar ID correction 9/4:** the live Power BI Agent Skills SharePoint item is internal item `612` with title `Agent Skills in Power BI | Ep. 67 | Principal GPM`; it contains no ADO ID. The earlier reported `236804` came from a conversational export and is not present in the live record. Weekly reconciliation now reads direct Graph fields and treats SharePoint item IDs as internal only.
- **Ron Chang short 237660 — delivered 8/31:** final delivery was emailed to Swetha, Terence, and Alyssa and recorded in ADO. Sara's work is complete; the Fabric Production Tracker now shows `Waiting` with ownership assigned to Fabric publishing / the ADO owner until they close the item.
- **Status:** assessed 7/20; iteration paths cleaned up
- **What:** Reviewed both ADO Shows (#174864 FTTF, #224267 FEI) for outstanding work.
- **Done 9/2 — PR #230 feedback resolved:** fixed Golnaz's remaining backward-scheduling issue so reverse-derived intermediate milestones honor weekday alignment, the full timeline recalculates forward consistently, and impossible working-day anchors are rejected. Added regression coverage, pushed commit `793f0db`, resolved the review thread, and emailed Golnaz the PR link and re-review request because she does not monitor GitHub.
...
- **Episode 233550 automation re-enabled 9/3:** the temporary suppression expired after the September 1 updated edit was shared for Yitzhak's review. The Episode remains tagged `waiting for final approval`.
